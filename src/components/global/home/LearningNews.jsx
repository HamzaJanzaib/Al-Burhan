        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs font-bold text-secondary uppercase tracking-[0.2em]">Latest Updates</span>
            </div>
          ...
          <Link href="/enroll">
            <motion.button 
              ...
              whileTap={{ scale: 0.95 }}
              className="group flex items-center gap-3 px-8 py-4 rounded-full bg-white border border-slate-100 text-primary font-bold shadow-sm hover:shadow-xl hover:border-primary/20 transition-all cursor-pointer"
            >
              Learn More <ArrowRight size={20} className="group-hover:translate-x-1.5 transition-transform" />
            </motion.button>
          </Link>
        </div>
            ...
            return (
              <motion.div
                ...
                className={`relative group cursor-pointer rounded-[32px] border transition-all duration-500
                  ${isActive 
                    ? 'bg-white border-primary/10 shadow-[0_20px_50px_rgba(13,92,92,0.1)] py-10 px-8' 
                    : 'bg-white/40 border-transparent hover:border-slate-200 py-8 px-8'
                  }`}
                layout
              >
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                  <div className="flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-12">
                    <div className={`flex items-center gap-2 text-sm font-bold tracking-wider transition-colors duration-300 ${isActive ? 'text-secondary' : 'text-slate-400'}`}>
                      <Calendar size={14} />
                      {item.date}
                    </div>
                    <div className="h-4 w-px bg-slate-200 hidden md:block" />
                    <h3 className={`text-xl md:text-2xl font-bold transition-colors duration-300 ${isActive ? 'text-primary' : 'text-slate-800'}`}>
                      {item.title}
                    </h3>
                  </div>

                  <div className="flex items-center gap-4">
                    <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${isActive ? 'bg-secondary/20 text-secondary' : 'bg-slate-100 text-slate-500'}`}>
                      {item.category}
                    </span>
