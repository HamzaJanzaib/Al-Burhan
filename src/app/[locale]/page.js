        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
            <div className="max-w-2xl text-left">
              ...
              <h2 className="text-4xl md:text-6xl font-black text-slate-900 leading-[1.1]">
                Nurturing the <span className="text-primary italic">Soul</span>{" "}
                <br /> through Divine Knowledge
              </h2>
            </div>
            <Link href="/service">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {homeServices.map((service, i) => (
              <div
                key={i}
                className="group p-10 rounded-[40px] border border-slate-100 bg-background hover:bg-primary hover:border-primary transition-all duration-500 shadow-sm hover:shadow-[0_30px_60px_rgba(13,92,92,0.15)] cursor-pointer"
              >
                <div className="w-14 h-14 rounded-2xl bg-primary/5 flex items-center justify-center mb-8 group-hover:bg-background/20 transition-colors shadow-sm">
                  {service.icon}
                </div>
                <h3 className="text-2xl font-bold mb-4 text-slate-900 group-hover:text-white transition-colors">
                  {service.title}
                </h3>
                <p className="text-slate-500 text-base leading-relaxed group-hover:text-white/80 transition-colors font-medium">
                  {service.desc}
                </p>
              </div>
            ))}
      ...
      <section className="py-28 bg-primary px-6 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          <h2 className="text-5xl md:text-7xl font-black text-white mb-10 leading-tight">
            Begin Your Spiritual <br /> Journey with{" "}
            <span className="text-secondary">AL Burhan</span>
          </h2>
          <p className="text-white/80 text-xl mb-14 max-w-2xl mx-auto font-medium">
            ...
            Start today with a free trial class and see the difference in your
