...
import { BookOpen, Heart, Languages, ShieldCheck, Sparkles, Star, ChevronRight, PlayCircle } from "lucide-react";
...
const courses = [
  ...
  },
  {
    ...
    features: ["Grammar & Vocab", "Quranic Translation", "Conversational Skills"],
  },
  {
    id: "kids",
    ...
    <div className="min-h-screen bg-background overflow-hidden">
      ...
      <section className="relative h-[60vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
          ...
          >
            Authentic Islamic education designed for modern learners. 
            From fundamental Reading to advanced Tajweed and Hifz.
          </motion.p>
        </div>
      </section>
            ...
            >
              ...
              <div className="absolute inset-0 p-8 flex flex-col justify-end text-white transition-all duration-500">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-12 h-12 rounded-2xl bg-white/20 backdrop-blur-xl border border-white/30 flex items-center justify-center group-hover:bg-white group-hover:text-secondary transition-all">
                    {course.icon}
                  </div>
                  <h3 className="text-2xl font-black group-hover:text-primary-foreground">{course.title}</h3>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mb-8 opacity-0 group-hover:opacity-100 transition-all duration-500 delay-100">
                  {course.features.map(f => (
                    <span key={f} className="text-[10px] font-black uppercase tracking-widest px-3 py-1 bg-white/10 rounded-full border border-white/20">
                      {f}
                    </span>
                  ))}
                </div>
                ...
                >
                  Explore Course <ChevronRight size={18} className="text-secondary group-hover:text-white" />
                </Link>
              </div>
            </motion.div>
          ))}
